    ...
    <>
      <TableRow key={`${gameInfo._id}key`}>
        ...
        <TableCell align='center'>
          <Button onClick={handleOpen}>{t('statistic.more')}</Button>
        </TableCell>
      </TableRow>
      <Modal sx={{ marginTop: '700px' }} open={open} onClose={handleClose}>
        <Box>
          <WinStatistic
            startTime={gameInfo.result_time}
            endTime={null}
            length={gameInfo.text.length}
            errorChar={gameInfo.incorrect_input}
            correctChar={gameInfo.correct_input}
            text={gameInfo.text}
            currIndex={1}
            time={gameInfo.timer}
            isUserPage={true}
